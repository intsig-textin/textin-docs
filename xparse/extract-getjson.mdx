    ...
    ```json
    {
      ...
      "result": {                             // 主要结果对象
        "llm_json": {                         // prompt模式抽取的原始结果
          "确认日期": "2024/4/3",              // 用户自定义字段，由prompt决定
          "基金代码": "011892",                // 用户自定义字段，由prompt决定
          "持仓金额": "74178.80"               // 用户自定义字段，由prompt决定
        },
