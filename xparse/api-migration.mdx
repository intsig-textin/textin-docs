...
<Warning>
  **两个默认值发生反转**：`include_image_data`（原 `get_image` 默认返回）和 `pages`（原 `page_details` 默认返回）在新版默认值均为 `false`，需手动置 `true`。
</Warning>
...
| `get_image`<br/>（none/page/objects/both） | objects | `capabilities.include_image_data`<br/>（是否返回） | `false` | ⚠️ 默认反转，需显式 `true`；详见下方说明 |
...
<Tip>
  - **parse_mode 在新版的默认值**：旧版默认 `scan`；新版 parse/sync 接口默认值以官方文档为准（通常为 `auto` 或 `scan`）
  - **force_engine 是新增的"引擎选择"维度**（textin/mineru/paddle_ocr/textin_gui），不是 `parse_mode` 的替代品，两者可同时设置
</Tip>
...
| `page_id` | `page_number` | ⚠️ 页码起始可能不同（见下节） |
...
<Steps>
  ...
  <Step title="评估引擎选择">
    评估是否需要 `force_engine` 选择特定引擎（新增能力，与 `parse_mode` 独立）
  </Step>
