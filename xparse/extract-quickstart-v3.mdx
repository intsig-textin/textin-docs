...
TextIn xParse现已推出的全新版本的文档抽取API（v3）。在文档抽取中，您可以自定义指定抽取模式（JSON schema）以及抽取模式中的字段和类型，系统会根据您的抽取模式进行抽取。如果您对文档抽取输出的结果字段类型有要求，例如区分文本和数字，那么文档抽取会非常有用。

通过定义JSON schema，文档抽取兼顾了抽取输出结果的稳定性，以及定义字段的灵活性。
...
在定义要抽取的数据时，您需要为每个字段提供一个名称，以及确定该字段的类型。您还可以添加可选的字段描述为大模型提供更多的上下文。字段名称和字段描述均可作为抽取指导，帮助文档抽取准确了解需要从文档中查找和提取哪些信息。字段名称和描述越具体、越具有描述性，文档抽取就越能准确地识别和抽取文档中的正确数据。
...
请注意，在JSON schema中array、object、enum类型均支持层级嵌套结构，以便于抽取如表格类的数据或根据枚举值进行判断。但目前文档抽取仅支持最多不超过3级的嵌套，例如：
