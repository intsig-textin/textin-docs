...
TextIn xParse现已推出的全新版本的文档抽取API（v3）。在文档抽取中，您可以自定义抽取配置（JSON schema），指定您要抽取的字段名称、类型和字段描述，系统会根据您定义的配置进行抽取。如果您对文档抽取输出的结果字段类型有要求，例如区分文本和数字，那么文档抽取会非常有用。
...
| 50012   | LLM Engine Faield (大模型引擎错误)  |   