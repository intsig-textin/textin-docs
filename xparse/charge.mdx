...
在Web平台使用时，可以在页面左下角账号信息处看到当前拥有的总额度和已消耗额度。
...
同时我们为每位新注册的用户朋友**免费赠送100页**使用额度！以便于您自行上传文档进行解析和抽取，验证TextIn xParse智能文档解析在您实际业务场景中的表现。
...
- 文档解析会根据您实际解析的文件页数进行计费扣减。
  - 您可以通过**page_start**和**page_count**这两个参数控制要解析的页数，详情见[**快速启动-URL参数说明**](https://docs.textin.com/xparse/parse-quickstart)。
  - 文档解析
  - 我们具体是通过哪个字段来判断计费数的，这个计费数怎么算出来的
  - 不额外再收费的情况

<Tip>
  文档智能抽取功能在Web平台使用**限时免费**！您可以充分体验
</Tip>

### 通过API使用

通过API使用时，仍然是根据实际解析和抽取的文件页数进行计费扣减。您可以通过以下字段进行核验

- 文档解析：[**返回JSON结构说明**](https://docs.textin.com/xparse/parse-getjson)；**success_count**字段
- 文档抽取：[**返回JSON结构说明**](https://docs.textin.com/xparse/extract-getjson)；**page_count**字段

与Web平台使用不同的是：无论何种文件，每次成功调用API进行解析或抽取后均会产生计费扣减。

另外还需要特殊注意的是：由于文档抽取API整合了文档解析处理能力和大模型语义理解能力，会先对文档进行解析处理再进行语义理解抽取，因此在计费上：文档抽取API的消耗是文档解析API的2倍。这样做是为了更好的保障文档抽取的结果准确性，以及可以提供精确的原文坐标，便于对结果做快速复核校验。

- 例如一份10页的文档：单独调用文档解析API时消耗页数额度为10页，单独调用文档抽取API时消耗页数额度为20页。如果是T币扣减，则单独使用文档解析扣减0.5T币，单独使用文档抽取扣减1T币。
