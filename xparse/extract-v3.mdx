...
```json
  "schema": {
    ...
    "properties": {
      "storeName": {
        "type": ["string", "null"],
        "description": "门店全称，需包含品牌与门店名称"
      },
      "isStoreName": {
        "type": ["enum", "null"],
        "enum": ["Y", "N"],
        "description": "提取的门店名称与“XX省XX市XX影城XX店”75% 及以上相似则为 Y，否则 N"
      },
      "orderNo": {
        "type": ["string", "null"],
        "description": "电影票订单号：取电影名称右侧的数字串"
      },
      "amount": {
        "type": ["number", "null"],
        "description": "订单实收 / 实付金额或票价，纯数值"
      },
      "time": {
        "type": ["date", "null"],
        "description": "票据发生时间，格式 yyyy-MM-dd HH:mm:ss"
      },
      "isGood": {
        "type": ["enum", "null"],
        "enum": ["Y", "N"],
        "description": "是否存在商品或电影信息：有为 Y，无为 N"
      }
    },
    "required": [
      "storeName",
...
抽取字段property是一个key-value形式的键值对。其key在properties中被定义和使用，包括name、type、description这三个关键字段要素，其value则是实际抽取所得到的结果。
...
为了获得最佳性能，保障抽取的精度和速度，在JSON schema中包含的最低层级字段数量限制总计不应超过100个。
...
调用[**API**](https://docs.landing.ai/api-reference/tools/agentic-document-extraction)即可提取数据。首先创建要抽取字段的 JSON schema，之后您可以在API调用中使用它。

您也可以参考以下文件和输入输出示例，快速验证并将文档抽取v3接入到您的系统和应用流程中。
...
#### extracted_schema：key-value形式的抽取结果

key与传入的schema结构一致，value为对应的抽取结果。

#### citations：抽取结果的详细信息，包含坐标位置

- **key**：结构与value一致，不做额外平铺展开
- **bounding_regions**：返回value对应的相对坐标，8个值的数组
  - **page_number**：段落所在文件实际的页码，从1开始，有offset
  - **text**：文本框所在区域的文字
- **value**：当前key全部抽取结果
...
使用JSON shcema定义抽取架构为文档抽取API拓展了额外的用法，例如您可以根据文档类型对文档进行分类并分别抽取数据。

假设您在一家金融机构工作，需要从贷款申请表中抽取一组数据，并从损益表中抽取另一组数据。您可以为每个文档分配一个类别，然后根据该文档的类别分别使用不同的抽取架构抽取数据。

在使用文档抽取v3时，您可以使用`enum`类型字段来标识文档类型，并通过2次调用API实现“先分类，再抽取”的效果。
...
```json
  "schema": {
    ...
    "properties": {
      "document_type": {
        "type": ["enum", "null"],
        "enum": ["Document Type 1", "Document Type 2", "Document Type 3"],
        "description": "文档分类，如果XXX则为Document Type 1，如果XXX则为Document Type 2，如果XXX则为Document Type 3"
      }
    },
    "required": [
      "document_type"
...
假设您需要对三种类型的文件做抽取：护照、发票和其他。您需要抽取的字段数据取决于文件类型。以下示例代码向您展示如何定义文档类型、每种文档类型要抽取的字段，以及实际对文档进行分类和抽取的代码。
